...
def read(timeout=4.0):
    """one JSON packet — console.log chatter rides the same pipe as
    bare text and is read past (the ast_probe's own law)."""
    r, _, _ = select.select([p.stdout], [], [], timeout)
    if not r: return None
    line = p.stdout.readline()
    if not line: return None               # EOF — the child is gone
    try:
        return json.loads(line)
    except Exception:
        return read(timeout)               # chatter — keep reading

def frame(dt=0.05, hits=None):
    send({"t": "tick", "dt": dt, "keys": {}, "chars": "",
          "hits": hits or []})
    while True:
        f = read()
