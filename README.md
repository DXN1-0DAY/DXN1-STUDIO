...
![version](https://img.shields.io/badge/version-3.1.110-8b5cf6?style=flat-square)
...
| `:git [log [n] \| graph [n] \| branch \| tag]` | the repo's truth in one breath — branch, uncommitted count, the last commit's name; `:git log [n]` walks the memory; `:git graph [n]` draws the shape lane by lane on the rail; `:git branch` names the locals, the current starred; `:git tag` counts the milestones (read-only; refuses honestly without git) |
