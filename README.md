...
![version](https://img.shields.io/badge/version-3.1.3-8b5cf6?style=flat-square)
...
| `:git [log [n] \| branch]` | the repo's truth in one breath — branch, uncommitted count, the last commit's name; `:git log [n]` walks the memory; `:git branch` names the locals, the current starred (read-only; refuses honestly without git) |
