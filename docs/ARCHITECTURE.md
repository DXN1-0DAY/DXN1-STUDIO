...
| `scripts/smoke_v590.py` (Xvfb) | 12 checks: the desk grows to fit — the chooser printing every number (es at 100% real, a seeded pack at 0% real in its row), the desk opening no narrower than what it packed (>= 680 and >= its requested width), the preview ratcheting out for a 120-char translated string and never shrinking back (the high-water mark, recorded in the shared helper's `_fit_size` since v2.63), the audit's closing lines naming the whole verb family (lang check <file> · lang pack <code> · lang diff <code>), source + doc agreement, and the palette audit regression (23) |
| `scripts/smoke_v600.py` (Xvfb) | 26 checks: the checkup comes home and learns to heal — `lang fix` as a verb (bare usage naming the dry-run, en refused, built-ins read-only, unknown languages named, findings repaired by name with the file verified on disk, already-clean answering with the kept count, unsafe strings surviving, an active language re-activating live), the desk running its own checkup (verdict empty until asked, button + Ctrl+T, an import running it automatically, red for findings with the full render reaching the terminal, green for clean at the honest 99% after a legal drop, an unsafe string turning it red again), width accounting round two (the hub fitting its 972px request, the old 940 fixed geometry proven to clip), source + doc agreement, and the palette audit regression (23) |
| `scripts/smoke_v610.py` (Xvfb) | 20 checks: a safety net for the fix — `lang fix` announcing and writing the `.bak`, `lang unfix` restoring byte-for-byte and consuming the backup (second unfix refused, gates matching lang fix, an active language re-activating live), the desk's Apply-safe-fixes button cutting dead weight with the verdict going green on its own (unsafe strings surviving, nothing-to-fix honest), the shared `geom.fit_to_content` helper growing past its floor and holding a ratchet high-water mark, all eight converted windows verified in source, source + doc agreement, and the palette audit regression (23) |
| `scripts/smoke_v620.py` (Xvfb) | 8 checks: width accounting round four — the after_idle contract (queued ≠ applied, applied once the event loop runs), two real converted windows opening at their floors (unit converter 560x460, text case 560x420), all ten conversions verified in source (with the self.win-vs-self.winfo substring trap handled), source + doc agreement, and the palette audit regression (23) |
| `scripts/smoke_v630.py` (Xvfb) | 16 checks: the sweep completes and the ledger goes public — three newly converted windows opening at their floors (macro panel 520x460, character map 640x480, theme gallery 760x600), the fleet audit (every fixed-geometry window in the package fit-covered, zero naked), the desk and preview riding the shared helper with the v2.59 custom ratchets retired, agent.py's inline v2.60 heal gone, `lang report` end to end (bare verb printing the whole table inline, one row per non-en pack sorted worst-first, the verdict naming full and seeded packs, a dest file landing with the verdict, never overwriting, an unwritable dest answered honestly), help + source + doc agreement (all 24 round-five riders commented), and the palette audit regression (23) |
| `scripts/smoke_v640.py` (Xvfb) | 13 checks: the studio keeps its windows — `tools windows` end to end (the honest empty answer, a real tool window listed numbered with transient marked and honest geometry, raise by index and by case-insensitive title substring, close by index really destroying the window, an ambiguous substring refused with the candidates named, an index past the list refused, close all destroying only the transient window and reporting what was spared, the usage line when the action has no target), the lang report header naming the studio version, and the palette audit regression (23) |
| `scripts/smoke_v650.py` (Xvfb) | 43 checks: the windows learn to mind their places — `tools cascade` stacks two parked tool windows from the top-left 28px apart with sizes kept and reports the stack, `tools tile` deals them into a screen-filling 2x1 grid (resized to cell, margin honoured, fully on-screen) and reports the grid shape, both honest when none open; the open-windows chip quiet at zero, "1 open" muted, "6 open" amber, signature cache holding, click running the honest listing (the VERB dispatch, not a shell echo), `chip windows` posting the menu with its 4 hint rows (Motion wired, tip closer armed, rows list/cascade/tile/close-all), registry + alias + honest unknown-chip answer, a menu row invoked for real, the unpost poller retiring the tip, scribe and sesave menus carrying hints; the pure-math regressions (cascade wrap inside 800x600, tiny-screen tile where the screen wins over the minimum cell, the single-tile full-screen rect); docs consistency (FEATURES/ARCHITECTURE/CHANGELOG/version); and the palette audit regression (23) |
| `scripts/smoke_v660.py` (Xvfb) | 28 checks: the window manager's memory — the bare verb teaching its usage, an empty desk refused honestly, save reporting what it remembers and persisting through the app config, bare-name restore moving both windows back EXACTLY, a name with spaces saving and restoring via `restore <name>`, the book listing every layout with its windows, a closed window reported missing by name without being conjured, unknown names and double-forgets answered honestly, forget all reporting the count (including 0 on an empty book), the pure-engine regression (capture fidelity, the 12-name LRU cap, exact match beating a substring impostor), docs consistency, and the palette audit regression (23) |
| `scripts/smoke_v670.py` (Xvfb) | 30 checks: the desk sees through walls — the layering engine as pure math (every spelling of sixty percent, every spelling of solid, the ends clamping so a window can never be ghosted invisible, junk refused without raising), `ghost` end to end (setting the level with percent and fraction spellings, the bare report changing nothing, a junk level refused with usage and the level kept, off restoring solid, missing targets named), `pin` reporting honestly in both directions (the readback contract noted — a bare Xvfb keeps no crown), the listing wearing `· NN%` and `· transient` with the pinned marker only when the WM kept the state, the footer teaching the new verbs, the layering proven skin-deep (titles untouched, raise still matches by name), the focused window wearing its marker when focus can be asked, the windows chip menu remembering the desk (the honest empty row, hints on every command row, the auto-named save row dispatching the real verb, the recall row appearing fresh and restoring for real, a second save walking the name forward, forgetting emptying the menu's book), every deps row carrying a hint, docs consistency, and the palette audit regression (23) |
| `scripts/smoke_v680.py` (Xvfb) | 35 checks: the layouts remember the layers — the honest read as pure math (a window that will not say is assumed ordinary, a banana is no crown and no ghost), the ghost level reading back, the capture carrying alpha + topmost, counts earned only from well-typed values, `apply_layout` handing back four-tuples with None for layers a snapshot never stated (old layouts and junk touch nothing), a solid desk saving without a layering suffix, a ghosted desk saving with `(1 ghosted)`, the list marking layered books only, `show` previewing the header with counts and marking what is open, an unknown name and a bare show answered honestly, restore re-ghosting and re-solidifying with the per-window note and the exact geometry back, a v2.66-era snapshot moving geometry without touching skins, find narrowing with an honest N-of-M header case-insensitively and missing honestly with the full list pointed at, the full listing unchanged by find, the tooltip speaking the desk as it IS and keeping up with the layering, the wins chip's Enter binding being the live one, docs consistency, and the palette audit regression (23) |
| `scripts/smoke_v690.py` (Xvfb) | 22 checks: the desk admits its drift — the pure engine (in place exact, moved with both geometries named, missing named never conjured, unbooked marking what the layout never knew, junk tolerated on both sides without raising, substring matching agreeing with the restore), the verb (bare usage, unknown honest, a fresh save diffing clean, the drift named after the window wanders and a stray appears, diff proven read-only, the restore agreeing with what the diff saw, the desk diffing clean again once recalled and the stray closed), docs consistency, and the palette audit regression (23) |
| `scripts/smoke_v700.py` (Xvfb) | 13 checks: the chip menu admits the drift — an aligned desk marked `as saved` with the hint explaining the mark, every command row still carrying a hint, a wandered desk marked `drifted` fresh at every post, a window closed behind the layout's back counting as drift, a book that cannot be read making no claim (no mark, an honest 0-window count for junk), the marked row still dispatching the real restore, docs consistency, and the palette audit regression (23) |
