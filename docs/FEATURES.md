...
## The Desk Sees Through Walls (v2.67.0)

| Feature | Where | What it does |
|---|---|---|
| `tools windows ghost <n|title> <level|off>` · `pin <n|title>` | `geom.parse_alpha()` · `geom.focused_toplevel()` + terminal verb | The layering verbs: `ghost` fades a tool window see-through (60, 60% and 0.6 all mean 60%; off — or solid, or full — restores 100%; the ends clamp so a window can never be ghosted into invisibility and lost) so a cheat-sheet can float over the editor without hiding it; bare `ghost <n|title>` reports the current level without touching it; `pin` toggles stays-above-everything for a window you keep glancing at. Neither verb ever touches a title — layouts and raise/close match by name, so the layering is only ever skin-deep. The listing wears what it can honestly know: `· NN%` for ghosted, `· pinned` when the window manager kept the crown, `· focused` for the window holding the keyboard |
| The windows chip menu remembers the desk | `app.py` `_wins_menu_entries()` · `_wins_menu_save_layout()` | The chip's right-click menu grows a layouts section, read fresh from the config store at every post: one recall row per remembered layout (`Recall layout 'desk' (3 windows)` — clicking dispatches the real restore), an honest "No layouts saved yet" row when the book is empty, and a `Save desk layout…` row that snapshots the desk under the next free auto name (desk, desk 2, desk 3 …) by dispatching the real verb, whose report says which name landed. Saving and recalling a desk never needs the terminal at all |
| The deps rows explain themselves | `app.py` `_deps_menu_entries()` | The severity-colored chip menu rows now carry hover hints that say why and what happens next — the repair row hands the missing imports to the fix queue, the copy row puts a ready pip line on the clipboard, the fresh rescan bypasses the cache because it is slower but the truth, the watch toggle lights the chip the moment an import goes missing, and the chip rescan redraws the verdict without a new scan. The colors say hurry; the hints say why |

## The Layouts Remember the Layers (v2.68.0)

| Feature | Where | What it does |
|---|---|---|
| Layered layouts — `tools layout save <name>` / `tools layout <name>` now carry the skin | `geom.window_layer()` · `geom.layer_counts()` + `capture_layout()` / `apply_layout()` | The window manager's memory goes skin-deep: snapshots now record each window's ghost level and pin alongside its geometry, so a desk saved ghosted recalls ghosted and a saved crown recalls pinned. The restore report speaks the layering per window (`Cheatsheet · ghosted to 40% · pinned`), a snapshot saved SOLID actively restores solid (`solid again` — said once, only when it changed something), and a snapshot that does not say — a v2.66-era layout, or junk values — touches nothing, because an old layout keeps meaning what it always meant and junk is never a license to repaint a window. The save report counts the layers up front (`remembers 3 windows (1 ghosted, 1 pinned)`), `list` marks layered books, and `window_layer` is the ONE honest read the listing, the capture and the tooltip all share |
| `tools layout show <name>` | terminal verb | A recall you can read before you run it: the whole snapshot, window by window — title, geometry, ghost level, pin — and whether each window is open right now (`· open now` / `· not open`), without touching a single window. Unknown names answered honestly, the bare verb teaches its usage |
| `tools windows find <text>` | terminal verb | Ctrl-F for the desk: the honest listing narrowed to the windows whose title carries the text, case-insensitive, with an honest `N of M open` header and all the markers (transient, ghost level, pin) riding the narrowed lines. A needle nothing carries is answered with the miss and the full list pointed at; the bare verb teaches its usage |
| `tools layout rename <old> <new>` | terminal verb | The book edits itself without touching the desk: the snapshot, its windows and its layers move to the new name untouched (`'desk' is now 'evening' (3 windows, untouched)`), and renaming never silently overwrites — a new name that already holds a layout is refused with the fix named (forget it first) |
| `tools layout diff <name>` | `geom.layout_drift()` + terminal verb | The desk admits its drift: the saved layout is compared against the live desk — what still stands exactly where the layout left it (`3 of 5 windows still in place`), what wandered (`moved: Chart — saved 300x200+10+10, now 500x300+40+40`, both honest), what is not open (named, never conjured), and what is open that the layout never knew (`unbooked: Stray (200x100+5+5)`). The same exact-first, substring-second matching the restore uses, so the diff never disagrees with what a recall would actually move — and it is read-only: not a single window is touched. A desk that matches exactly says so; junk on either side is tolerated, never fatal |
| `tools layout diff <a> <b>` — book to book | `geom.layout_compare()` + terminal verb | Two SAVED layouts, compared: the windows standing identically in both, the windows that changed (`changed: Chart — 300x200+10+10 → 500x300+40+40 · ghost solid → 40% · pin off → on` — geometry AND layer deltas named, because a layout is skin and place alike), and the windows only one book knows (`only in 'day'`). Titles match case-insensitively and exactly — book to book, no substring guessing. The single-name live-diff form is tried FIRST (the full argument as a layout name wins, so names with spaces keep working); identical books agree out loud; junk on either side is tolerated, never fatal |
| The chip menu admits the drift | `app.py` `_wins_menu_entries()` | The windows chip menu's recall rows are marked FRESH at every post, read through `geom.layout_drift()` — `· as saved` when the live desk still matches the layout exactly, `· drifted` when anything moved, was closed behind the layout's back, or strayed in unbooked — and the row's hint explains the mark. A book that cannot be read makes no claim at all: no well-formed snapshot, no mark, no honest window-count lies — but the row still recalls, because a mark is garnish and a recall is a promise |
| The live chip tooltip | `app.py` `_chip_tip_fn()` · `_wins_tip_text()` | The open-windows chip's hover help is now COMPUTED at hover, not frozen at startup: `Open tool windows — 3 open · 1 ghosted · 1 pinned — click lists them…`, speaking the desk as it IS. A callable that raises or answers nothing means no tooltip at all — garnish never breaks a hover |
