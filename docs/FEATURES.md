...
| Context menu | right-click the branch chip, app `_git_menu_entries()` | The lane's actions in one themed menu: Open Source Control, Commit graph, Stage all changes (through the visible terminal runner), Copy branch name (clipboard + toast), Rescan — repo rows appear only when a repository is watched, so a plain folder gets the honest two-row menu |
...
| Chip tooltips | app `_chip_tip()` | Hover any statusbar chip (git, deps, session autosave, scribe) and a quiet themed tooltip explains what it is and what clicking it does |
